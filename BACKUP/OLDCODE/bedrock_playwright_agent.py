...
"""
            ...
            using_registry_xpath = False  # Track if we're using registry XPath (skip tree climbing & discovery)
            ...
            try:
                ...
                if optimized_selector_used and selector.startswith("xpath="):
                    ...
                    if xpath_count == 0:
                        ...
                        optimized_selector_used = False
                        using_registry_xpath = False  # Reset flag on fallback
                        all_matches = await self.page.locator(selector).all()
                    elif xpath_count > 1:
            ...
            try:
                ...
                # 🔒 CRITICAL: If using registry XPath, SKIP tree climbing (manual XPath = source of truth)
                if using_registry_xpath and len(visible_matches) == 1:
                    # Registry XPath found exactly 1 match - use it directly, skip tree climbing
                    logger.info(f"  🔒 Using registry XPath directly - skipping tree climbing (manual XPath)")
                    ...
                    for i, match in enumerate(visible_matches):
                        ...
                        # If element is NOT directly interactive AND NOT using registry XPath, climb DOM tree
                    ...
                    # 🔒 CRITICAL: If using registry XPath, skip tree climbing (already handled above)
                    if not using_registry_xpath:
                        ...
                        if not is_interactive:
                            ...
                            if len(candidates) == 1:
                                logger.warning(f"  ⚠️ Climbed {max_depth} levels, no interactive ancestor found. Element may not be clickable!")
                    else:
                        # Using registry XPath - already handled above, candidates should be empty
                        pass
                    ...
                    if click_succeeded:
                        ...
                        if chosen_locator:
                            try:
                                ...
                                # 🔒 CRITICAL: Skip discovery tracking if using registry XPath (manual XPath = source of truth)
                                if not using_registry_xpath:
                                    ...
                                    logger.info(f"  🔒 Skipped discovery tracking (using registry XPath - manual XPath is source of truth)")
            ...
            # TOTP Detection: Check if this is a TOTP step (needed before registry check)
            step_metadata = self.parsed_steps.get(self.current_step_number, {})
            step_text = step_metadata.get('text', '')
            
            # 🔒 CRITICAL: Check element registry FIRST (same as browser_click)
            # Extract element name from step text for registry lookup (e.g., "password" from "enter password")
            # Registry uses element names (e.g., "Password", "PASSWORD input"), not selectors (e.g., "input[type='password']")
            element_name_for_registry = None
            if step_text:
                import re
                step_lower = step_text.lower()
                
                # Priority 1: Extract specific field names (most reliable)
                # Look for common field names first (before generic patterns)
                ...
                # Priority 2: If no specific field found, try pattern extraction
                if not element_name_for_registry:
                    # Pattern: "enter password", "fill password", "type password", etc.
                    # Match only single word after "enter/fill/type" (non-greedy, stop at first space or "as")
                ...
                logger.info(f"  🔍 Extracted element name from step text: '{element_name_for_registry}' (from: '{step_text[:60]}...')")
            
            # Try registry lookup with element name first, then fallback to selector
            registry_selector = None
            if element_name_for_registry:
                # Try exact match first
                registry_selector = self._check_element_registry(element_name_for_registry)
                
                # If exact match failed, try variations (e.g., "password" -> "PASSWORD input", "Password")
                if not registry_selector:
                    # Try capitalized version
                    registry_selector = self._check_element_registry(element_name_for_registry.capitalize())
                
                # If still no match, try with " input" suffix (e.g., "password" -> "password input")
                if not registry_selector:
                    registry_selector = self._check_element_registry(f"{element_name_for_registry} input")
                
                # Try uppercase version
                if not registry_selector:
                    registry_selector = self._check_element_registry(element_name_for_registry.upper())
                
                if registry_selector:
                    logger.info(f"  📋 Found registry entry for '{element_name_for_registry}'")
            
            # If element name lookup failed, try selector-based lookup (for backward compatibility)
            if not registry_selector:
                registry_selector = self._check_element_registry(selector)
            ...
            if registry_selector:
                selector = registry_selector
                # If registry selector is an XPath, we're using manual XPath
                if selector.startswith("xpath="):
                    using_registry_xpath = True
            ...
            # 🔒 CRITICAL: Only use TOTP fallback if NOT using registry XPath (registry XPath = source of truth)
            ...
            # Execute
            # Try registry selector first, with fallback to original if it fails
            try:
                ...
                # Registry selector failed, fall back to original selector
