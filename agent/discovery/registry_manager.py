...
class RegistryManager:
    ...
    async def save_discoveries(self, discoveries: List[Dict], current_url: str, 
        ...
        try:
            ...
            for discovery_url, page_discoveries in discoveries_by_url.items():
                ...
                # Group discoveries by context (modal vs main-page)
                modal_discoveries = [d for d in page_discoveries if d.get('context') == 'modal']
                main_page_discoveries = [d for d in page_discoveries if d.get('context') != 'modal']
                
                logger.info(f"  📊 Grouped by context: {len(modal_discoveries)} modal, {len(main_page_discoveries)} main-page")
                ...
                # Save main-page discoveries to main page file
                if main_page_discoveries:
                    logger.info(f"  💾 Saving {len(main_page_discoveries)} main-page discoveries to {page}")
    ...
    async def _save_discoveries_to_page(self, domain: str, page: str, page_discoveries: List[Dict], 
        ...
        """
        Save discoveries to a specific page registry file
        Args:
            domain: Domain name (e.g., 'hub-stage.datacommons.cancer.gov')
            page: Page name (e.g., 'data-submissions' or 'data-submissions-modal')
            page_discoveries: List of discovery dicts to save
            discovery_url: URL where discoveries were made
            preserve_manual: If True, preserve existing XPaths
        """
