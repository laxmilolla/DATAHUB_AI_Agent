...
@bp_excel.route('/api/excel-execution/<execution_id>/steps', methods=['GET'])
def get_execution_steps_with_status(execution_id):
    """
    Get execution steps with status (passed/failed/clicked/etc).
    Combines Excel steps with execution results.
    
    Returns:
        JSON with steps array, each step having status and details
    """
    try:
        import re
        project_root = current_app.config.get('PROJECT_ROOT', Path.cwd())
        ...
        # Get excel_id from execution
        excel_id = execution_data.get('excel_id')
        if not excel_id:
            return jsonify({'error': 'Execution is not from Excel file'}), 400
        ...
        # Parse stdout to extract step statuses
        stdout = execution_data.get('stdout', '')
        step_statuses = {}
        
        # Pattern to match step status messages
        # Examples: "✅ Step 1: Click succeeded", "❌ Step 2: Failed", "🔐 Step 8: Generated TOTP"
        patterns = [
            (r'✅\s*Step\s+(\d+[a-z]?):\s*(.+?)(?:\n|$)', 'success'),
            (r'❌\s*Step\s+(\d+[a-z]?):\s*(.+?)(?:\n|$)', 'failed'),
            (r'⚠️\s*Step\s+(\d+[a-z]?):\s*(.+?)(?:\n|$)', 'warning'),
            (r'🔐\s*Step\s+(\d+[a-z]?):\s*(.+?)(?:\n|$)', 'totp'),
            (r'✅\s*Step\s+(\d+[a-z]?):\s*Clicked\s+(\w+)', 'clicked'),
            (r'✅\s*Step\s+(\d+[a-z]?):\s*Filled\s+input', 'filled'),
            (r'✅\s*Step\s+(\d+[a-z]?):\s*Navigation\s+completed', 'navigated'),
        ]
        
        for pattern, status_type in patterns:
            matches = re.finditer(pattern, stdout, re.MULTILINE | re.IGNORECASE)
            for match in matches:
                step_num = match.group(1).strip()
                message = match.group(2).strip() if len(match.groups()) > 1 else match.group(0)
                
                if step_num not in step_statuses:
                    step_statuses[step_num] = {
                        'status': status_type,
                        'messages': []
                    }
                
                step_statuses[step_num]['messages'].append(message)
                # Update status if we found a failure
                if status_type == 'failed':
                    step_statuses[step_num]['status'] = 'failed'
        
        # Combine Excel steps with execution status
        steps_with_status = []
        for idx, row in df.iterrows():
            step = str(row.get('step', idx + 1)).strip()
            url = str(row.get('url', '')).strip() if pd.notna(row.get('url')) else ''
            ...
            text_value = str(row.get('text_value', '')).strip() if pd.notna(row.get('text_value')) else ''
            wait_time_raw = row.get('wait_time', None)
            wait_time = None if pd.isna(wait_time_raw) else (float(wait_time_raw) if wait_time_raw is not None else None)
            functions = str(row.get('functions', '')).strip() if pd.notna(row.get('functions')) else ''
            is_optional = str(row.get('optional', '')).strip().lower() in ['true', 'yes', '1', 'y']
            modal = str(row.get('modal', '')).strip() if pd.notna(row.get('modal')) else None
            
            # Get status for this step
            step_status_info = step_statuses.get(step, {})
            status = step_status_info.get('status', 'pending')
            status_messages = step_status_info.get('messages', [])
            
            # Determine status display
            status_display = 'pending'
            status_icon = '⏳'
            if status == 'success' or status == 'clicked' or status == 'filled' or status == 'navigated':
            ...
            steps_with_status.append({
                ...
                'functions': functions if functions else None,
                'optional': is_optional,
                'modal': modal if modal else None,
                'status': status_display,
                'status_icon': status_icon,
                'status_messages': status_messages,
                'has_details': len(status_messages) > 0
            })
        ...
        return jsonify({
            ...
            'passed_steps': sum(1 for s in steps_with_status if s['status'] == 'passed'),
            'failed_steps': sum(1 for s in steps_with_status if s['status'] == 'failed'),
            'pending_steps': sum(1 for s in steps_with_status if s['status'] == 'pending')
        }), 200
    
    except Exception as e:
        import traceback
        print(f"Error getting execution steps: {e}")
        print(traceback.format_exc())
        return jsonify({'error': str(e)}), 500
