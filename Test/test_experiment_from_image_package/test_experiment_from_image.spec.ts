...
Generated from: test_experiment_from_image.xlsx*/
...
test('test_excel_generated', async ({ page }) => {
    ...
    try {
            ...
            try {
                ...
                await element.fill('Laxmi_Al_test@yahoo.com');
                console.log(`✅ Step 5: Filled input with Laxmi_Al_test@yahoo.com`);
            ...
            try {
                ...
                // Submit after TOTP - wait for redirect to complete
                try {
                    const initialUrl = page.url();
                    // Wait for URL to change (redirect) or page to load
                    await page.waitForFunction((initial) => page.url() !== initial, initialUrl, { timeout: 10000 });
                    console.log(`✅ Step 10: Redirect detected, waiting for page to load...`);
                    await page.waitForLoadState('networkidle');
                    await page.waitForLoadState('domcontentloaded');
                    console.log(`✅ Step 10: Page loaded after redirect`);
                } catch (redirect_error) {
                    // If redirect doesn't happen, just wait for page to settle
                    console.log(`⚠️  Step 10: No redirect detected, waiting for page to settle...`);
                    await page.waitForLoadState('domcontentloaded');
            ...
            try {
                ...
                const normalizeUrl = (u: string) => u.split('?')[0].split('#')[0].replace(/\/$/, '');
                if (normalizeUrl(currentPageUrl) !== normalizeUrl(expectedUrl)) {
                    console.log(`⚠️  Step 13: Not on expected page. Current: ${currentPageUrl}, Expected: ${expectedUrl}. Navigating...`);
                    await page.goto(expectedUrl);
                    await page.waitForLoadState('networkidle');
                    await page.waitForLoadState('domcontentloaded');
                    console.log(`✅ Step 13: Navigated to expected page: ${expectedUrl}`);
                }
            } catch (nav_error) {
                console.log(`⚠️  Step 13: Navigation check failed, continuing anyway: ${nav_error}`);
            }
            try {
                let element;
                // Try registry lookup first
                try {
                    ...
                    element = page.locator(selector).nth(0);
                    await element.waitFor({ state: 'visible', timeout: 10000 });
                    console.log(`✅ Step 13: Using registry element_id: ID_8fcff2a7`);
                } catch (registry_error) {
                    // Registry lookup failed - test must fail
                    console.log(`❌ Step 13: Registry lookup failed for element_id ID_8fcff2a7: ${registry_error}`);
                    await page.screenshot({ path: 'storage/screenshots/pw_step13_button_registry_failed.png' });
                ...
                await page.waitForTimeout(1000);  // Wait after click
                console.log(`✅ Step 13: Clicked button`);
                await page.screenshot({ path: 'storage/screenshots/pw_step13_button.png' });
            } catch (e) {
                console.log(`❌ Step 13: Failed to click button: ${e}`);
                await page.screenshot({ path: 'storage/screenshots/pw_step13_button_failed.png' });
            ...
            try {
                ...
                try {
                    ...
                    element = page.locator(selector).nth(0);
                    await element.waitFor({ state: 'visible', timeout: 10000 });
                    console.log(`✅ Step 14: Using registry element_id: ID_38253381`);
                } catch (registry_error) {
                    // Registry lookup failed - test must fail
                    console.log(`❌ Step 14: Registry lookup failed for element_id ID_38253381: ${registry_error}`);
            ...
            try {
                ...
                try {
                    // Use Excel URL (from row) for registry lookup - this is the page where element should be
                    const lookupUrl = 'https://hub-stage.datacommons.cancer.gov/data-submissions' || page.url();
                    const xpath = getXpathById('ID_f26aa3f3', lookupUrl);
                    const selector = `xpath=${xpath}`;
                    element = page.locator(selector).nth(0);
                    await element.waitFor({ state: 'visible', timeout: 10000 });
                    console.log(`✅ Step 15: Using registry element_id: ID_f26aa3f3`);
                } catch (registry_error) {
                    // Registry lookup failed - test must fail
                    console.log(`❌ Step 15: Registry lookup failed for element_id ID_f26aa3f3: ${registry_error}`);
                    await page.screenshot({ path: 'storage/screenshots/pw_step15_input_registry_failed.png' });
                    throw new Error(`Registry lookup failed for element_id ID_f26aa3f3: ${registry_error}`);
                }
                // Scroll into view if needed
                try {
                    await element.scrollIntoViewIfNeeded();
                ...
                await page.waitForTimeout(1000);  // Wait after click
                console.log(`✅ Step 15: Clicked input`);
                await page.screenshot({ path: 'storage/screenshots/pw_step15_input.png' });
            } catch (e) {
                console.log(`❌ Step 15: Failed to click input: ${e}`);
                await page.screenshot({ path: 'storage/screenshots/pw_step15_input_failed.png' });
            ...
            try {
                ...
                try {
                    // Use Excel URL (from row) for registry lookup - this is the page where element should be
                    const lookupUrl = 'https://hub-stage.datacommons.cancer.gov/data-submissions' || page.url();
                    const xpath = getXpathById('ID_ce7aacba', lookupUrl);
                    const selector = `xpath=${xpath}`;
                    element = page.locator(selector).nth(0);
                    await element.waitFor({ state: 'visible', timeout: 10000 });
                    console.log(`✅ Step 16: Using registry element_id: ID_ce7aacba`);
                } catch (registry_error) {
                    // Registry lookup failed - test must fail
                    console.log(`❌ Step 16: Registry lookup failed for element_id ID_ce7aacba: ${registry_error}`);
                    await page.screenshot({ path: 'storage/screenshots/pw_step16_input_registry_failed.png' });
                    throw new Error(`Registry lookup failed for element_id ID_ce7aacba: ${registry_error}`);
                }
                await element.fill('');
                console.log(`✅ Step 16: Filled input with `);
                await page.waitForTimeout(500);  // Wait after fill
                await page.screenshot({ path: 'storage/screenshots/pw_step16_input.png' });
            } catch (e) {
                console.log(`❌ Step 16: Failed to fill input: ${e}`);
                await page.screenshot({ path: 'storage/screenshots/pw_step16_input_failed.png' });
                criticalFailures.push(`Step 16: Fill failed`);
            }
