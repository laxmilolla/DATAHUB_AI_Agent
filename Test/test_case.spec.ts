...
test('test_excel_generated', async ({ page }) => {
    ...
    try {
            ...
            try {
                const currentPageUrl = page.url();
                const expectedUrl = 'https://auth.nih.gov/CertAuthV3/forms/nihl/LoginMFA.aspx';
                // Normalize URLs for comparison (remove trailing slashes, query params)
                const normalizeUrl = (u: string) => u.split('?')[0].split('#')[0].replace(/\/$/, '');
                if (normalizeUrl(currentPageUrl) !== normalizeUrl(expectedUrl)) {
            ...
            try {
                ...
                try {
                    ...
                    // Press Enter to submit the form immediately after TOTP entry
                    await element.press('Enter');
                    console.log(`✅ Step 8: Pressed Enter to submit form`);
                    // Wait for redirect after form submission
                    try {
                        const initialUrl = page.url();
                        await page.waitForFunction((initial) => page.url() !== initial, initialUrl, { timeout: 10000 });
                        console.log(`✅ Step 8: Redirect detected after Enter key`);
                        await page.waitForLoadState('networkidle');
                        await page.waitForLoadState('domcontentloaded');
                    } catch (redirect_error) {
                        console.log(`⚠️  Step 8: No redirect after Enter key, waiting for page to settle...`);
                        await page.waitForLoadState('domcontentloaded');
            ...
            try {
                ...
                const normalizeUrl = (u: string) => u.split('?')[0].split('#')[0].replace(/\/$/, '');
                if (normalizeUrl(currentPageUrl) !== normalizeUrl(expectedUrl)) {
                    console.log(`⚠️  Step 9: Not on expected page. Current: ${currentPageUrl}, Expected: ${expectedUrl}. Navigating...`);
                    await page.goto(expectedUrl);
                    await page.waitForLoadState('networkidle');
                    await page.waitForLoadState('domcontentloaded');
                    console.log(`✅ Step 9: Navigated to expected page: ${expectedUrl}`);
                }
            } catch (nav_error) {
                console.log(`⚠️  Step 9: Navigation check failed, continuing anyway: ${nav_error}`);
            }
            try {
                let element;
                // Try registry lookup first
                try {
                    ...
                    element = page.locator(selector).nth(0);
                    await element.waitFor({ state: 'visible', timeout: 10000 });
                    console.log(`✅ Step 9: Using registry element_id: ID_8fcff2a7`);
                } catch (registry_error) {
                    // Registry lookup failed - test must fail
                    console.log(`❌ Step 9: Registry lookup failed for element_id ID_8fcff2a7: ${registry_error}`);
            ...
            try {
                ...
                try {
                    ...
                    element = page.locator(selector).nth(0);
                    await element.waitFor({ state: 'visible', timeout: 10000 });
                    console.log(`✅ Step 10: Using registry element_id: ID_38253381`);
                } catch (registry_error) {
                    // Registry lookup failed - test must fail
                    console.log(`❌ Step 10: Registry lookup failed for element_id ID_38253381: ${registry_error}`);
            ...
            try {
                ...
                const normalizeUrl = (u: string) => u.split('?')[0].split('#')[0].replace(/\/$/, '');
                if (normalizeUrl(currentPageUrl) !== normalizeUrl(expectedUrl)) {
                    console.log(`⚠️  Step 11: Not on expected page. Current: ${currentPageUrl}, Expected: ${expectedUrl}. Navigating...`);
                    await page.goto(expectedUrl);
                    await page.waitForLoadState('networkidle');
                    await page.waitForLoadState('domcontentloaded');
                    console.log(`✅ Step 11: Navigated to expected page: ${expectedUrl}`);
                }
            } catch (nav_error) {
                console.log(`⚠️  Step 11: Navigation check failed, continuing anyway: ${nav_error}`);
            }
            try {
                let element;
                // Try registry lookup first
                try {
                    // Use Excel URL (from row) for registry lookup - this is the page where element should be
                    const lookupUrl = 'https://hub-stage.datacommons.cancer.gov/data-submissions' || page.url();
                    const xpath = getXpathById('ID_07a4c7a7', lookupUrl);
                    const selector = `xpath=${xpath}`;
                    element = page.locator(selector).nth(0);
                    await element.waitFor({ state: 'visible', timeout: 10000 });
                    console.log(`✅ Step 11: Using registry element_id: ID_07a4c7a7`);
                } catch (registry_error) {
                    // Registry lookup failed - test must fail
                    console.log(`❌ Step 11: Registry lookup failed for element_id ID_07a4c7a7: ${registry_error}`);
                    await page.screenshot({ path: 'storage/screenshots/pw_step11_input_registry_failed.png' });
                    throw new Error(`Registry lookup failed for element_id ID_07a4c7a7: ${registry_error}`);
                }
                // Scroll into view if needed
                try {
                    await element.scrollIntoViewIfNeeded();
                ...
                await page.waitForTimeout(1000);  // Wait after click
                console.log(`✅ Step 11: Clicked input`);
                await page.screenshot({ path: 'storage/screenshots/pw_step11_input.png' });
            } catch (e) {
                console.log(`❌ Step 11: Failed to click input: ${e}`);
                await page.screenshot({ path: 'storage/screenshots/pw_step11_input_failed.png' });
            ...
            try {
                ...
                const normalizeUrl = (u: string) => u.split('?')[0].split('#')[0].replace(/\/$/, '');
                if (normalizeUrl(currentPageUrl) !== normalizeUrl(expectedUrl)) {
                    console.log(`⚠️  Step 12: Not on expected page. Current: ${currentPageUrl}, Expected: ${expectedUrl}. Navigating...`);
                    await page.goto(expectedUrl);
                    await page.waitForLoadState('networkidle');
                    await page.waitForLoadState('domcontentloaded');
                    console.log(`✅ Step 12: Navigated to expected page: ${expectedUrl}`);
                }
            } catch (nav_error) {
                console.log(`⚠️  Step 12: Navigation check failed, continuing anyway: ${nav_error}`);
            }
            try {
                let element;
                // Try registry lookup first
                try {
                    ...
                    element = page.locator(selector).nth(0);
                    await element.waitFor({ state: 'visible', timeout: 10000 });
                    console.log(`✅ Step 12: Using registry element_id: ID_ce7aacba`);
                } catch (registry_error) {
                    // Registry lookup failed - test must fail
                    console.log(`❌ Step 12: Registry lookup failed for element_id ID_ce7aacba: ${registry_error}`);
                    await page.screenshot({ path: 'storage/screenshots/pw_step12_input_registry_failed.png' });
                    throw new Error(`Registry lookup failed for element_id ID_ce7aacba: ${registry_error}`);
                }
                await element.fill('${Timestamp}');
                console.log(`✅ Step 12: Filled input with ${Timestamp}`);
                await page.waitForTimeout(500);  // Wait after fill
                await page.screenshot({ path: 'storage/screenshots/pw_step12_input.png' });
            } catch (e) {
                console.log(`❌ Step 12: Failed to fill input: ${e}`);
                await page.screenshot({ path: 'storage/screenshots/pw_step12_input_failed.png' });
                criticalFailures.push(`Step 12: Fill failed`);
            }
